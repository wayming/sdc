...
func (swim *SAPageWorkItemManager) OnProcessError(wi IWorkItem, err error) error {
	...
	if !ok {
		return fmt.Errorf("Failed to convert the work item to SAPageExporter work item")
	}
...
func (swb *SAPageWorkBuilder) NewWorker() (IWorker, error) {
	...
	return &SAPageExporter{logger: swb.logger, reader: NewHttpReader(&client)}, nil
}

func (se *SAPageExporter) Init() error {
	dateStr := time.Now().Format("20060102")
	se.exportDir = config.DataDir + "/" + dateStr

	if err := common.CreateDirIfNotExists(se.exportDir); err != nil {
		return err
	}
	return nil
}

func (se *SAPageExporter) Do(wi IWorkItem) error {
	swi, ok := wi.(SAPageWorkItem)
	if !ok {
		return fmt.Errorf("Failed to convert the work item to SAPageExporter work item")
	}
...
func (se *SAPageExporter) Done() error {
	// Do nothing
	return nil
}

func (se *SAPageExporter) ExportSAPage(url string, file string) error {
