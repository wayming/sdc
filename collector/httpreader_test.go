...
func TestHttpProxyReader_Read_NotFound(t *testing.T) {
	type fields struct {
		ProxyFile string
	}
	type args struct {
	...
	tests := []struct {
		...
		{
			name: "TestHttpProxyReader_Read_NotFound",
			fields: fields{
				ProxyFile: os.Getenv("SDC_HOME") + "/data/proxies10.txt",
			},
			args: args{
				url:    "https://stockanalysis.com/stocks/NotFound",
				params: nil,
			},
			wantErr: true,
		},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			reader := collector.NewHttpProxyReader(cacheManager, PROXY_CACHE_TEST_KEY, "100")
			_, err := reader.Read(tt.args.url, tt.args.params)
			if (err != nil) != tt.wantErr {
				t.Errorf("HttpProxyReader.Read() error = %v, wantErr %v", err, tt.wantErr)
				return
			}

			if e, ok := err.(collector.HttpServerError); !ok {
				t.Error("HttpProxyReader.Read() got no error. want HttpServerError")
			} else {
				if e.StatusCode() != collector.HTTP_ERROR_NOT_FOUND {
					t.Errorf("HttpProxyReader.Read() got http code = %d, want %d", e.StatusCode(), collector.HTTP_ERROR_NOT_FOUND)
				}
			}
		})
	}
